...
#ADLS configuration 
spark.conf.set(
  "fs.azure.account.key.storagehospitalresource.dfs.core.windows.net",
  <<vault_secret_key>>
)
...
gold_fact = "abfss://<<container>>@<<storageaccount_name>>.dfs.core.windows.net/"
...
fact_final.write.format("delta").mode("overwrite").save(gold_fact + "/patient_fact")
...
print("Fact rows:", spark.read.format("delta").load(gold_fact + "/patient_fact").count())